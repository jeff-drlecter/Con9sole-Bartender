# Con9sole-Bartender — Fly.io 版（Global sync, Duplicate + Ping）
import os
...
async def duplicate_section(client: discord.Client, guild: discord.Guild, game_name: str) -> str:
    ...
    for name in FALLBACK_CHANNELS.get("text", []):
        if not any(isinstance(c, discord.TextChannel) and c.name == name for c in new_cat.channels):
            await guild.create_text_channel(name, category=new_cat, overwrites=make_private_overwrites(guild, [new_role], admin_roles))

    # 後備 Voice
    for name in FALLBACK_CHANNELS.get("voice", []):
        if not any(isinstance(c, discord.VoiceChannel) and c.name == name for c in new_cat.channels):
            await guild.create_voice_channel(name, category=new_cat, overwrites=make_private_overwrites(guild, [new_role], admin_roles))
